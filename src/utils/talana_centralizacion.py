...
from src.utils.dates import get_last_month
...
def main(data=None, context=None):
    """
    Función principal para procesar la centralización contable del mes anterior.
    """
    last_year, last_month = get_last_month()

    url = f"https://talana.com/es/api/centralizacion-contable-generica?ano={last_year}&mes={last_month}"
    output_filename = "centralizacion_contable_output.ndjson"
    remote_filename = "CentralizacionContable.ndjson"

    try:
        process_data(url, output_filename, remote_filename)
    except Exception as e:
        logger.error(f"Error procesando centralización contable: {e}")

    logger.info("Centralización contable procesada correctamente.")
...
def procesar():
    ...
    """
    logger.info("=== Ejecutando talana_centralizacion.procesar() ===")
    main()
