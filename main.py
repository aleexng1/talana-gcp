...
# Configuración de loggingpip install requests google-cloud-storage
...
def sanitize_field_name(name):
    ...
    # Reemplaza todos los caracteres especiales por '_'
...
def rename_duplicate_sueldo_mensual(data):
    if isinstance(data, dict):
        # Detecta claves duplicadas ignorando mayúsculas/minúsculas
        keys_lower = [k.lower() for k in data.keys()]
        if keys_lower.count('sueldo mensual') > 1:
            count = 0
            new_items = {}
            for k in list(data.keys()):
                if k.lower() == 'sueldo mensual':
                    if count == 0:
                        count += 1
                        continue
                    new_key = f"Sueldo_mensual_duplicated_{count}"
                    new_items[new_key] = data.pop(k)
                    count += 1
    ...
    with open(filename, "w", encoding="utf-8") as json_file:
        for i, item in enumerate(data):
            try:
                ...
                logging.error(f"Error writing record #{i}: {e}")
                logging.debug(f"Record with error: {json.dumps(item, indent=2, ensure_ascii=False)}")
...
if __name__ == "__main__": 
    ...
    if not USERNAME or not PASSWORD:
        logging.error("Las credenciales de Talana no están definidas en las variables de entorno.")
        exit(1)
    # Validación de configuración de GCP
    if not PROJECT_ID or not BUCKET_NAME:
        logging.error("La configuración de Google Cloud Storage no está definida correctamente.")
        exit(1)
