...
def fetch_data(url, token):
    ...
    while url:
        try:
            ...
            response = session.get(url, headers={"Authorization": f"Token {token}"}, timeout=30)
            response.raise_for_status()
            data = response.json()
...
def main(data, context=None):
    ...
    endpoints = [
        #Liquidaciones
        (f"https://talana.com/es/api/liquidaciones/?periodo__mes={last_month}&periodo__ano={last_year}", "liquidaciones_output.ndjson", "Liquidaciones.ndjson")
...
def procesar():
    ...
    logging.info("=== Ejecutando talana_liquidaciones.procesar() ===")
    main(data=None)
